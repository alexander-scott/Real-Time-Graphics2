...
bool TargaTexture::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext)
{
	...
	// Setup the description of the TargaTexture.
	...
	// Copy the targa image data into the TargaTexture.
	...
	// Create the shader resource View for the TargaTexture.
	...
	// Generate mipmaps for this TargaTexture.
	deviceContext->GenerateMips(_textureView);
	
	// Release the targa image data now that the image data has been loaded into the TargaTexture.
	delete [] _targaData;
	_targaData = 0;
...
bool TargaTexture::LoadTarga(char* filename, int& height, int& width)
{
